...
def rerank_cross_encoder(
    ...
    if not jina_key:
        raise ValueError("JINA_API_KEY không tồn tại trong file cấu hình .env.")
...
def rerank(
    ...
    # Fallback tự động nếu thiếu API Key cho Cross-Encoder
    if method == "cross_encoder" and not jina_key:
        print("[WARNING] JINA_API_KEY khong duoc thiet lap. Tu dong chuyen sang thuat toan RRF.")
        method = "rrf"
    ...
    if method == "cross_encoder":
        try:
            ...
            print(f"[WARNING] Cross-Encoder reranking gap loi ({err_msg}). Fallback sang RRF.")
            method = "rrf"
    ...
    if method == "mmr":
        try:
            ...
            # Sinh vector embedding cho candidates nếu chưa có
            for c in candidates:
            ...
            err_msg = str(e).encode("ascii", errors="ignore").decode("ascii")
            print(f"[WARNING] MMR reranking gap loi ({err_msg}). Fallback sang RRF.")
            method = "rrf"

    if method == "rrf":
        # RRF cần danh sách của các ranked lists.
        # Nếu candidates là 1 list phẳng, ta xem nó như 1 danh sách duy nhất.
        return rerank_rrf([candidates], top_k)
    else:
        raise ValueError(f"Phương pháp rerank không xác định: {method}")
